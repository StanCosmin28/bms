...
import offerData from "../Model/promotionalSectionData";
export default function PromotionalSection() {
  const [currentSlide, setCurrentSlide] = useState(0);

  const nextSlide = () => {
    setCurrentSlide((prev) => (prev === offerData.length - 1 ? 0 : prev + 1));
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev === 0 ? offerData.length - 1 : prev - 1));
  };
